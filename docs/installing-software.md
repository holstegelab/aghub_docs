...
3. Install or run it on AGHub.

If you have not set up transfer yet, first read
[Use of Research Drive](research-drive.md).

## Recommended: install with conda-pack

Use this when you need extra Python/R tools beyond the default AGHub
environment.
...
!!! note "No Conda locally?"

    If Conda is not installed on your local machine, use
    [Miniforge](https://github.com/conda-forge/miniforge#mambaforge).

### On AGHub

1. Create the target directory and unpack:
...
!!! warning "No SLURM submission inside the container"

    You cannot run SLURM submission commands (for example `sbatch`) from

...
The steps can take a little time because several SURF systems synchronize in
the background. Once the account is set up, normal login is straightforward.
...
!!! warning "Password setup link expires"

    The SURFcua password setup link is valid for 12 hours and can be used
    only once. Do not close the browser tab until the password setup has
    finished.
...
1. Open the AGHub collaboration invitation email and accept the invitation.
2. Click **Login** and search for your identity provider.
3. Choose one login route:
...
4. Complete any 2FA step requested by your institute, SRAM, or EduID.

![Screen showing the institute search box](images/broad_select.png)

??? info "If your institute is not available in SRAM"

    If your institute appears but login fails with a message about SRAM not
    being activated, contact your local ICT or helpdesk. Include a screenshot
    of the error message and ask them to enable the institute Identity
    Provider for SURF Research Access Management. You can refer them to the
    [SRAM service provider information](https://dashboard.surfconext.nl/apps/8164/saml20_sp/about).

    If institutional login cannot be enabled quickly, use EduID as the
    fallback. Search for `EduID` in the identity provider box. If you do not
    already have an EduID account, create one first. EduID uses its own
    app-based 2FA during account setup.
...
!!! note "Synchronization can take ~20 minutes"

    Synchronization from SRAM to the AGHub account system can take about
    20 minutes. If you still cannot continue after one hour, contact an
    AGHub administrator.

## Step 2: Set Your AGHub Password

SURFcua is the SURF account portal used for AGHub login and SSH key
management. After your AGHub account is created, you receive two emails:

- `SURFcua new login <username> created` — contains your AGHub username and useful links.
- `SURFcua Update Your login` — contains the one-time password setup link.

Use the setup link from `SURFcua Update Your login`.

1. Open the password setup link and click **Click here to proceed**.
2. Enter a new password twice. Use at least 12 characters with lowercase, uppercase, number, and special character.
3. When the page says your account has been updated, click **Back to Application**.

This password is your AGHub/SURFcua portal password. You will use it when
logging in through the AGHub doornode and when adding or removing SSH keys in
the SURFcua portal.

## Step 3: Register 2FA for SURFcua

AGHub uses a TOTP token as the second factor for SURFcua login.
...
4. Add a new account (usually a `+`, **Add account**, or **Scan QR code** button) and choose to scan a QR code. Allow camera access if asked.
5. Point the camera at the QR code shown by SURFcua. The app creates a new entry and shows a 6-digit code that changes every ~30 seconds.
6. Type the current code into the SURFcua page and continue.
7. Accept the SURF CUA End Usage Agreement if prompted.

??? info "What if I cannot use a phone?"

    TOTP works the same way on a desktop. Install a desktop TOTP application
    such as KeePassXC and use its TOTP function instead of a phone app.
    Store any recovery information in a safe place.

    The AGHub/SURFcua 2FA is separate from any 2FA you may already use for
    your institute, SRAM, or EduID account.
...
AGHub login uses SSH key authentication. Your computer keeps the *private*
key; the matching *public* key is uploaded once to SURFcua.

!!! danger "Never upload your private key"

    Only ever upload the file ending in `.pub`. The matching private key
    (without `.pub`) must stay on your own computer.
...
??? info "Supported SSH key types and key removal"
    ...
    SURFcua supports common OpenSSH public key types: `ssh-ed25519`,
    `ssh-rsa`, ECDSA keys, and security-key based OpenSSH keys. You can
    remove old or unused keys from the same **SSH Keys** page.
...
After the account and SSH key have synchronized, connect from the computer
that holds your private SSH key:

```bash
ssh sram-aghub-[first-initial][last-name]@doornode.hpcv.surf.nl
```

Replace `[first-initial][last-name]` with your AGHub username suffix.
For example, if your assigned username is `sram-aghub-jdoe`:
...
| Problem | Fix |
| --- | --- |
| Password setup link expired | Ask an AGHub administrator or SURF support for a new SURFcua update link. |
| Authenticator code is rejected | Wait for the next code and try again. Check that your phone or computer clock is set automatically. |
| SSH login still fails after upload | Wait for synchronization. Confirm you uploaded the public key, not the private key. If it still fails after ~1 hour, contact an AGHub administrator. |
